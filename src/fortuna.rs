...
pub struct Fortuna {
    ...
    reseed_count: u32,
    last_reseed_time: time::Duration,
}
...
impl Fortuna {
    ...
    pub fn new_unseeded() -> Fortuna {
        Fortuna {
            ...
            reseed_count: 0,
            last_reseed_time: Duration::ZERO,
        }
    }
...
impl Rng for Fortuna {
    ...
    fn fill_bytes(&mut self, dest: &mut [u8]) {
        // Reseed if necessary
        let now = OffsetDateTime::now_utc() - OffsetDateTime::UNIX_EPOCH; //precise_time_s();
        if self.pool[0].count >= MIN_POOL_SIZE
            && now - self.last_reseed_time > Duration::new(0, 100000000)
        {
            //0.1 {
            self.reseed_count += 1;
...
impl<'a> SeedableRng<&'a [u8]> for Fortuna {
    ...
    fn reseed(&mut self, seed: &'a [u8]) {
        self.reseed_count += 1;
        self.last_reseed_time = OffsetDateTime::now_utc() - OffsetDateTime::UNIX_EPOCH; //precise_time_s();
